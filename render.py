...
class Render:
    def __init__(self, screen) -> None:
        ...
        '''
        self.screen = screen
        self.font = pygame.font.SysFont(None, SCREEN_HIGHT//20) # Размер шрифта зависит от размера экрана
    ...
    def draw_game_frame(self, frames, score = 0):
        ...
        pygame.draw.rect(self.screen, GRAY, pygame.Rect(0,0,SCREEN_WIDTH,SCREEN_HIGHT//20)) # для панели счета делаем подложку.
        text_img = self.font.render(f"SCORE: {score}", True, BLACK)
        self.screen.blit(text_img,(0,0))

    def draw_text(self, text, start_position = SCREEN_HIGHT//2): 
        '''
        ...
        indent = SCREEN_HIGHT // 20 # Вычисляем отступ
